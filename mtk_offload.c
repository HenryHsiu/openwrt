...
mtk_flow_hash_v4(struct flow_offload_tuple *tuple)
{
	...
	hash &= 0x7ff;
	hash *= 2;
	//printk(KERN_ALERT "-----origin hash=%d,   src_port=%d", hash, tuple->src_port);
		   ...
		   struct flow_offload_hw_path *dest)
{
	...
	printk(KERN_ALERT "sp=%d -> dp=%d,   new_sp=%d -> new_dp=%d", entry->ipv4_hnapt.sport, entry->ipv4_hnapt.dport, entry->ipv4_hnapt.new_sport, entry->ipv4_hnapt.new_dport);
...
mtk_check_hashcollision(struct mtk_eth *eth, u32 hash)
{
	...
	printk(KERN_ALERT "entry.bfib1.state=%d", entry.bfib1.state);
	      ...
	      struct mtk_foe_entry *entry)
{
	...
	printk(KERN_ALERT "Successfully write hash=%d to PPE table", hash);
}
		     ...
		     struct flow_offload_hw_path *dest)
{
	...
	printk(KERN_ALERT "=================== binding info  ===================");
	printk(KERN_ALERT "type=%d", type);
	...
	case AF_INET:
		...
		printk(KERN_ALERT "ohash=%d,  rhash=%d", ohash, rhash);
		break;
